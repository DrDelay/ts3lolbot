...
func main() {
	conn, err := ts3.Dial("5.45.102.5:10011", false) // TODO Config
	if err != nil {
...
func bot(conn *ts3.Conn) {
	...
	command(conn, "login ** **", true, false) // TODO: Config
	command(conn, "use 1", true, true)        // TODO: Config
	clientID := aliveTick(conn)
	command(conn, fmt.Sprintf("clientupdate client_nickname=%s", ts3.Quote("LoLBot")), false, true) // TODO: Config
	command(conn, fmt.Sprintf("clientmove clid=%d cid=%d", clientID, 10), false, true)              // TODO: Config
...
func aliveTick(conn *ts3.Conn) int {
	...
	clientID, parseErr := strconv.Atoi(payload["client_id"])
	if parseErr != nil {
